 ...
 */
{
  ...
  const mount = (store, registry, projectPanel) => {
    ...
    const grid = planner.grid.create({ root, surface });
    ...
    const dispatch = (/** @type {CommandIntent} */ command) => {
      ...
      if (command.command === "nudge") {
        ...
        const along = (
          /** @type {number} */ value,
          /** @type {number} */ delta,
        ) => planner.geo.stepTo(value, delta, step);
          ...
          for (const application of floor.applications || []) {
            ...
            if (group?.rect)
              return registry.dispatch({
                ...
                rect: {
                  ...group.rect,
                  x: along(group.rect.x, command.delta.x),
                  y: along(group.rect.y, command.delta.y),
                },
                coalesce: command.coalesce,
              });
            for (const type of application.deviceTypes || []) {
              ...
              if (instance)
                return registry.dispatch({
                  ...
                  action: "move",
                  id: command.id,
                  x: along(instance.x, command.delta.x),
                  y: along(instance.y, command.delta.y),
                  coalesce: command.coalesce,
                });
            }
          }
