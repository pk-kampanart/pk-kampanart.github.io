 ...
 */
{
  ...
  /** @param {Document} document @param {PlanDocument | null} plan */
  ...
  /** @param {Document} document @returns {PlanDocument} */
  ...
  const projectEntity = (document, schema, entity, value) => {
    ...
    const projected =
      entity === "group"
        ? planner.groups.withStrokeDefaults(
            /** @type {Group} */ (/** @type {unknown} */ (value)),
          )
        : value;
    dom.render(node, entity, /** @type {EntityRecord} */ (projected));
    const definition = definitionOf(schema, entity);
    ...
    for (const field of Object.keys(definition.properties || {})) {
      ...
      if (!child) continue;
      const children = projected[field];
