 ...
 */
{
  ...
  /** @param {PlanDocument} plan @param {string} typeId @param {() => string} ids @param {number} x @param {number} y */
  ...
  /** @param {PlanDocument} plan @param {string} typeId @param {string} name */
  ...
  /** @param {PlanDocument} plan @param {string} typeId */
  ...
  /** @param {PlanDocument} plan @param {string} instanceId @param {number} x @param {number} y */
  ...
  /** @param {PlanDocument} plan @param {string} instanceId */
  ...
  /** @param {PlanDocument} plan @param {string} typeId @param {string} targetId @param {"before" | "after"} [where] */
  const reorderType = (plan, typeId, targetId, where = "before") => {
    const next = clone(plan);
    for (const floor of next.floors)
      for (const application of floor.applications) {
        const types = application.deviceTypes;
        const from = types.findIndex((type) => type.id === typeId);
        const target = types.findIndex((type) => type.id === targetId);
        if (from < 0 || target < 0 || from === target) continue;
        const [type] = types.splice(from, 1);
        const destination = types.findIndex((item) => item.id === targetId);
  ...
  /** @param {PlanDocument} plan @param {string} typeId @param {string} applicationId @param {number} [index] */
  const moveType = (plan, typeId, applicationId, index) => {
    ...
    if (!source || !type || !target || sourceFloor !== targetFloor) return next;
