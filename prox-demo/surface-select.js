/**
 * Resolves pointer positions on painted surface entities for selection and
 * hover.
 ...
 */
{
  ...
  /** @param {Element} surface @param {MouseEvent} event */
  const surfacePointOf = (surface, event) => {
  ...
  /** @param {Element} surface @param {MouseEvent} event */
  const entityAt = (surface, event) => {
    const point = surfacePointOf(surface, event);
    const candidates = [];
  ...
  const create = (options) => {
    ...
    const pointOf = (event) => surfacePointOf(surface, event);
    ...
    const drawStart = (event) => {
      ...
      if (
        ...
        return;
      const { shape, point } = entityAt(surface, event);
      if (shape) return;
      drawing = { start: point };
    ...
    const drawEnd = (event) => {
      ...
      const drawn = planner.geo?.normalize?.({
        x: start.x,
        y: start.y,
        width: end.x - start.x,
      ...
      const rect = planner.geo.snapRect(drawn, planner.grid.stepFor(event));
      options.onDraw?.({
        floorId:
      ...
      (event) => {
        ...
        if (
          ...
          const detail = {
            ...
            floorId:
              document.documentElement.getAttribute("data-active-floor") || "",
            // Snapping rounds the gesture, never the drawing (ADR-0035); the
            // hit-test point above stays exact so resolution is unaffected.
            point: planner.geo.snapPoint(
              point,
              planner.grid.stepFor(/** @type {MouseEvent} */ (event)),
            ),
          };
          if (shape) {
            options.onPlace?.({
  ...
  planner.surfaceSelect = Object.freeze({ create, entityAt });
}
