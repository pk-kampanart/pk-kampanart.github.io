  ...
  const create = (options) => {
    ...
    const renderSurface = (plan) => {
      ...
      for (const rect of surface.querySelectorAll(
        ...
        rect.setAttribute(
          "aria-label",
          `${group.name}, ${Math.round(group.rect.width)} by ${Math.round(group.rect.height)} at ${Math.round(group.rect.x)}, ${Math.round(group.rect.y)}`,
        );
        const selected =
    ...
    const geometryOf = (current, at, event) => {
      ...
      return planner.geo.snapRect(drawn, planner.grid.stepFor(event));
    };
