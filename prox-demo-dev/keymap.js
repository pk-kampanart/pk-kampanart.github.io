 ...
 */
{
  ...
  const sources = [
    ...
    },
    {
      ...
      command: "reset-zoom",
      editable: false,
      description: "Reset zoom to the whole plan",
    },
    {
      keys: ["G"],
      command: "toggle-grid",
    ...
    },
    {
      ...
      command: "toggle-snap",
      editable: false,
      description: "Snap placement to the grid",
    },
    {
  ...
  const create = (options) => {
     ...
     * @param {string} key
     * @param {number} step
     * @param {boolean} [snap] round the result to the grid; the fine step is
     * the deliberate way off it, so it never does
     */
    const move = (key, step, snap = false) => {
      ...
      const grid = snap ? planner.grid.step() : 1;
      cursor = {
        x: planner.geo.stepTo(cursor.x, direction.x * step, grid),
        y: planner.geo.stepTo(cursor.y, direction.y * step, grid),
      };
      renderCursor();
