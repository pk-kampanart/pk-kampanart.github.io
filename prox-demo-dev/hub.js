 ...
 */
{
  ...
  const create = (options) => {
    ...
    const commands = {
      ...
      "move-cursor": (event) =>
        Boolean(gestures?.move(event.key, planner.grid.nudgeStep(), true)),
      "fine-move": (event) => Boolean(gestures?.move(event.key, 1, false)),
      "zoom-in": () => Boolean(dispatch({ command: "zoom-in" })),
      "zoom-out": () => Boolean(dispatch({ command: "zoom-out" })),
      "reset-zoom": () => Boolean(dispatch({ command: "reset-zoom" })),
      "toggle-grid": () => Boolean(dispatch({ command: "toggle-grid" })),
      "toggle-snap": () => Boolean(dispatch({ command: "toggle-snap" })),
    };
