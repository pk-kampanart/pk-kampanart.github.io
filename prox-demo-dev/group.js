 ...
 */
{
  ...
  /**
   * What a group's stroke controls show: resolved defaults, the color the
   * swatch displays, and whether that color is an override or the group's own.
   * @param {Group} group
   */
  const strokeSlots = (group) => {
    const stroke = strokeOf(group);
    return {
      strokeType: stroke.type,
      strokeWidth: stroke.width,
      strokeOpacity: Math.round(stroke.opacity * 100),
      strokeColorShown: stroke.color,
      strokeInherits: group.strokeColor ? "false" : "true",
      strokeOverride: group.strokeColor ? "override" : "inherits",
    };
  };
