 ...
 */
{
  ...
  const PANEL_COLLAPSED_KEY = "mesh-planner.prototype-panel-collapsed";
  const REVIEW_ENABLED_KEY = "mesh-planner.prototype-review-enabled";
  const steps = new Set(["prototype", "variants", "notes", "summary"]);
  let availableKeys = [];
  let reviewEnabled = true;
  let applyReviewChrome = () => {};
  ...
  const notifyReviewState = () => {
    for (const listener of reviewListeners) listener(reviewState());
  };

  const readReviewEnabled = () => {
    try {
  ...
  window.PlannerDevPrototypes = Object.freeze({
    get available() {
    ...
    setReviewEnabled(enabled) {
      ...
      try {
        ...
        // Storage denial only forgets the preference on reload.
      }
      applyReviewChrome();
      notifyReviewState();
    },
  ...
  style.textContent = `
    [data-part="prototype-fab"] {
      ...
      z-index: 1000;
      display: grid;
      min-width: min(24rem, calc(100vw - ${PANEL_EDGE * 2}px));
      max-width: calc(100vw - ${PANEL_EDGE * 2}px);
    ...
    [data-part="prototype-fab-title"] {
      grid-column: 1 / -1;
      text-align: center;
    }

    [data-part="prototype-fab"] button {
      width: auto;
      min-height: 30px;
      padding-inline: var(--space-3);
    }

    [data-part="prototype-fab"] [data-action="open-review"] {
      display: grid;
      min-width: 0;
      gap: var(--space-1);
    ...
    [data-part="prototype-fab-prototype"],
    [data-part="prototype-fab-variant"] {
      overflow: hidden;
    ...
    [data-part="prototype-fab"] [data-action="previous-variant"],
    [data-part="prototype-fab"] [data-action="next-variant"] {
      padding-inline: var(--space-2);
      font-size: 1.25rem;
    }

    [data-part="prototype-fab"] [data-part="prototype-variant-status"],
    [data-part="prototype-fab"] [data-part="prototype-variant-dots"] {
      grid-column: 1 / -1;
    }
  ...
  const start = async () => {
    ...
    const selectedPrototypes = defaultSelected
      ...
      : listedPrototypes;
    availableKeys = selectedPrototypes.map((prototype) => prototype.key);
    reviewEnabled = availableKeys.length > 0 && readReviewEnabled();
    notifyReviewState();
    if (availableKeys.length === 0) return;
    ...
    const variantIndex = active?.variants.indexOf(activeVariant) ?? -1;
    const previousVariant =
      active && variantIndex >= 0 && active.variants.length > 1
        ? active.variants[
            (variantIndex - 1 + active.variants.length) % active.variants.length
          ]
        : null;
    const nextVariant =
      active && variantIndex >= 0 && active.variants.length > 1
        ? active.variants[(variantIndex + 1) % active.variants.length]
        : null;
    const fab = document.createElement("section");
    fab.dataset.part = "prototype-fab";
    fab.setAttribute("aria-label", "Prototype review");
    fab.innerHTML = `
      <strong data-part="prototype-fab-title">Prototype review</strong>
      <button
        type="button"
        data-action="previous-variant"
        ${previousVariant ? `data-variant="${escape(previousVariant.key)}"` : "disabled"}
      ...
      <button
        ...
        aria-expanded="false"
      >
        <span data-part="prototype-fab-prototype">${escape(
          active?.title ||
            (requestedKey === "none" ? "Original" : "Prototype unavailable"),
        )}</span>
        <span data-part="prototype-fab-variant">${activeVariant ? `Variant: ${escape(activeVariant.name)}` : ""}</span>
      </button>
      <button
        type="button"
        data-action="next-variant"
        ${nextVariant ? `data-variant="${escape(nextVariant.key)}"` : "disabled"}
        aria-label="Next variant"
        aria-keyshortcuts="ArrowRight"
        title="Next variant (Right Arrow)"
      >→</button>
      <strong data-part="prototype-variant-status"></strong>
      <ol data-part="prototype-variant-dots" aria-label="Variant progress"></ol>`;
    fab.hidden = true;
    ...
    try {
      panelCollapsed =
        window.sessionStorage.getItem(PANEL_COLLAPSED_KEY) === "true";
    } catch {
      // Dev-only convenience; storage denial leaves the panel open.
    }
    applyReviewChrome = () => {
      if (!reviewEnabled) {
        panel.hidden = true;
      ...
      panel.hidden = panelCollapsed;
      fab.hidden = !panelCollapsed;
    };
    applyReviewChrome();
    ...
    const fabVariantStatus = fab.querySelector(
      '[data-part="prototype-variant-status"]',
    );
    const fabVariantDots = fab.querySelector(
      '[data-part="prototype-variant-dots"]',
    );
    ...
    /** @param {Element} status @param {Element} dots */
    const renderVariantProgress = (status, dots) => {
      const index = active?.variants.indexOf(activeVariant) ?? -1;
      dots.replaceChildren();
      if (!active || !activeVariant || index < 0) {
        status.textContent = "";
        return;
      }
      status.textContent = `Variant ${index + 1} of ${active.variants.length}`;
      active.variants.forEach((variant, variantIndex) => {
        const dot = document.createElement("li");
        dot.dataset.state = variantIndex === index ? "current" : "upcoming";
        dot.setAttribute(
          "aria-label",
          `Variant ${variantIndex + 1} of ${active.variants.length}: ${variant.name}`,
        );
        if (variantIndex === index) dot.setAttribute("aria-current", "step");
        dots.append(dot);
      });
    };
    ...
    const renderPrototypeProgress = () => {
      ...
      renderVariantProgress(fabVariantStatus, fabVariantDots);
    ...
    const renderVariantStep = () => {
      ...
      stepRoot.innerHTML = `
        <nav data-part="prototype-variant-navigation" aria-label="Prototype variants">
          <button type="button" data-variant="${escape(previous.key)}" aria-label="Previous variant" aria-keyshortcuts="ArrowLeft" title="Previous variant (Left Arrow)" ${active.variants.length > 1 ? "" : "disabled"}>Previous variant</button>
          <div>
            <strong data-part="prototype-variant-status"></strong>
            <ol data-part="prototype-variant-dots" aria-label="Variant progress"></ol>
          </div>
          <button type="button" data-variant="${escape(next.key)}" aria-label="Next variant" aria-keyshortcuts="ArrowRight" title="Next variant (Right Arrow)" ${active.variants.length > 1 ? "" : "disabled"}>Next variant</button>
        </nav>
      ...
      renderVariantProgress(
        stepRoot.querySelector('[data-part="prototype-variant-status"]'),
        stepRoot.querySelector('[data-part="prototype-variant-dots"]'),
      );
    };
    ...
    fab.addEventListener("click", (event) => {
      ...
      if (button?.dataset.variant) {
        if (active) navigate(active.key, button.dataset.variant);
        return;
      }
      panel.hidden = false;
      fab.hidden = true;
      try {
        window.sessionStorage.removeItem(PANEL_COLLAPSED_KEY);
      } catch {
        // Dev-only convenience; storage denial only affects the next reload.
      }
      fab.querySelector('[data-action="open-review"]')?.setAttribute(
        "aria-expanded",
        "true",
      );
      requestAnimationFrame(clampPanel);
    ...
    panel.addEventListener("click", async (event) => {
      ...
      if (button.dataset.action === "collapse") {
        ...
        fab.hidden = false;
        try {
          window.sessionStorage.setItem(PANEL_COLLAPSED_KEY, "true");
        } catch {
          // Dev-only convenience; storage denial only affects the next reload.
        }
        const opener = fab.querySelector('[data-action="open-review"]');
    ...
    window.addEventListener("keydown", (event) => {
      const reviewHasFocus = panel.hidden
        ? !fab.hidden
      ...
      const target =
        active.variants[
          (index + offset + active.variants.length) % active.variants.length
        ];
      const button = [
        ...(panel.hidden
          ? fab.querySelectorAll("[data-variant]")
          : panel.querySelectorAll("[data-variant]")),
      ].find(
        (candidate) => candidate.dataset.variant === target.key,
      );
      if (button) button.click();
      else navigate(active.key, target.key);
    });
