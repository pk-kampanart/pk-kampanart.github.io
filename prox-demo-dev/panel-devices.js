 ...
 */
{
  ...
  const create = (options) => {
    if (
      ...
      throw new TypeError(
        "device panel requires root, surface, store, and registry",
      );
    ...
    /** @param {CommandIntent} intent @param {unknown} [tag] */
    ...
    surface.addEventListener("dragover", (event) => {
      if (
        !event.dataTransfer?.types.includes("application/x-planner-device-type")
      )
        return;
    ...
    const endDrag = (event) => {
      ...
      const current = [
        ...surface.querySelectorAll('[data-part="surface-device"]'),
      ].find((item) => item.getAttribute("data-id") === drag.id);
    ...
    root.addEventListener("drop", (event) => {
      ...
      if (!draggedTypeId || !target || !targetId || targetId === draggedTypeId)
        return;
      event.preventDefault();
